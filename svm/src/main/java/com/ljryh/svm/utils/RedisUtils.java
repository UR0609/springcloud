...
public class RedisUtils {
     ...
     * @return
     */
    public List<svm_node> exampleToTFIDF(String example,String typeName){
        ...
        for (Term t : terms) {
            ...
            if (this.get(typeName + Constant.SVM_DATA + name) != null) {
                svmNode.index = Integer.parseInt(this.get(typeName + Constant.SVM_DATA + name).toString());
                svmNode.value = (double) Integer.parseInt(this.get(typeName + Constant.SVM_DATA_COUNT + name).toString())
                        / Integer.parseInt(this.get(typeName + Constant.SVM_TOTAL_COUNT).toString());
     ...
     * @return
     */
    public double getTargetValues(String typeName, String intention) {
        Object intentionData = this.get(typeName + Constant.SVM_INTENTION_DATA + intention);
        if (intentionData == null) {
            Object obj = this.get(typeName + Constant.SVM_INTENTION_COUNT);
            if (obj == null) {
                this.set(typeName + Constant.SVM_INTENTION_COUNT, 1.0);
                this.set(typeName + Constant.SVM_INTENTION_DATA + intention, 1.0);
                return 1.0;
            } else {
                this.set(typeName + Constant.SVM_INTENTION_COUNT, Double.parseDouble(obj.toString()) + 1);
                this.set(typeName + Constant.SVM_INTENTION_DATA + intention, Double.parseDouble(obj.toString()) + 1);
                return Double.parseDouble(obj.toString()) + 1;
            }
        } else {
            return Double.parseDouble(intentionData.toString());
        }
    }
}
